...
func usage() {
	...
	fmt.Fprintln(out)
	fmt.Fprintln(out, "  Randomly choose one of VARIANTs.")
	fmt.Fprintln(out)
	fmt.Fprintf(out, "%s [OPTION]... sample SIZE [FILE]\n", ProgName)
	fmt.Fprintln(out)
	fmt.Fprintln(out, "  Sample of lines with size SIZE from stdin or FILE if specified.")
	fmt.Fprintln(out)
	fmt.Fprintf(out, "%s [OPTION]... float\n", ProgName)
	fmt.Fprintln(out)
	fmt.Fprintln(out, "  Generates random float value 0 <= X < 1")
	fmt.Fprintln(out)
	fmt.Fprintf(out, "%s [OPTION]... int N\n", ProgName)
	fmt.Fprintln(out)
	fmt.Fprintln(out, "  Generates random integer value 0 <= X < N")
	fmt.Fprintln(out)
	fmt.Fprintf(out, "%s [OPTION]... stream\n", ProgName)
	fmt.Fprintln(out)
	fmt.Fprintln(out, "  Output infinite stream of random bytes.")
	fmt.Fprintln(out)
	fmt.Fprintf(out, "%s version\n", ProgName)
	fmt.Fprintln(out)
	fmt.Fprintln(out, "  Output program version and exit.")
	fmt.Fprintln(out)
